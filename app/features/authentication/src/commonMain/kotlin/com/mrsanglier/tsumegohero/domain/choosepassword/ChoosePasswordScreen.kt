...
import com.mrsanglier.tsumegohero.app.coreui.resources.choosePassword_topbar_title
import com.mrsanglier.tsumegohero.app.coreui.resources.illustration_password
...
@Composable
fun ChoosePasswordRoute(
    navScope: ChoosePasswordNavScope,
    viewModel: ChoosePasswordViewModel = koinViewModel(),
) {
    ...
    safeNavigation(
        ...
        when (navigation) {
            is ChoosePseudoDestination -> navScope.navigateToChoosePseudo(navigation)
        }
    }

    ChoosePasswordScreen(
        uiState = uiState,
        passwordTextUiField = viewModel.passwordTextUiField,
        navigateBack = navScope.navigateBack,
    )
}

@Composable
private fun ChoosePasswordScreen(
    uiState: ChoosePasswordViewModelState,
    passwordTextUiField: PasswordTextUiField,
    navigateBack: () -> Unit,
) {
    ...
    Scaffold(
        ...
        topBar = {
            THTopBar(
                title = THString.choosePassword_topbar_title.toTextSpec(),
        ...
        Column(
            ...
            Image(
                painter = painterResource(THDrawable.illustration_password),
            ...
            Column(
                ...
                passwordTextUiField.Composable(Modifier)
                THTheme.spacing.small.THVerticalSpacer()

                uiState.criteriaCells.chunked(2).forEach { chunk ->
                    Row {
                        chunk.getOrNull(0)?.Composable(Modifier.weight(1f))
                        chunk.getOrNull(1)?.Composable(Modifier.weight(1f))
                    }
                }
            }
        }
